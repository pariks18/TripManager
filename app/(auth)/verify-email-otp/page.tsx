            ...
            <div className="flex items-center justify-center gap-1 sm:gap-3" onPaste={handlePaste}>
              {otp.map((digit, index) => (
                <input
                  ...
                  className="w-8 sm:w-12 h-11 sm:h-14 bg-slate-950 border border-slate-800 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-center font-mono text-base sm:text-2xl font-extrabold text-emerald-400 rounded-xl sm:rounded-2xl transition-all outline-none shrink-0"
                  autoFocus={index === 0}
                />
              ))}
            </div>
