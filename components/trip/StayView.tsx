...
const PREDEFINED_STAY_AMENITIES = [
  'Towels',
  'Bedsheets',
  'Blankets',
  'Toiletries',
...
export const StayView: React.FC<StayViewProps> = React.memo(({
  ...
  const handleOpenEditModal = (stay: StayDetail) => {
    ...
    setNotes(stay.notes || '');
    setAvailableItems(stay.availableItems || []);
    setCustomItemInput('');
    setError('');
    setIsModalOpen(true);
  };
        ...
        <div className="space-y-4">
          {stays.map((stay) => (
            ...
            >
              ...
              {/* Available at this Stay Section */}
              <div className="bg-pink-50/60 border border-pink-100 p-3.5 rounded-2xl space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-extrabold text-pink-950 flex items-center gap-1.5">
                    <Hotel className="w-3.5 h-3.5 text-pink-600" /> Available at this Stay:
                  </span>
                  {isAdmin && (
                    <button
                      type="button"
                      onClick={() => handleOpenEditModal(stay)}
                      className="text-[10px] font-extrabold text-pink-700 hover:underline"
                    >
                      Edit Items
                    </button>
                  )}
                </div>
                {stay.availableItems && stay.availableItems.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    {stay.availableItems.map((item) => (
                      <span
                        key={item}
                        className="bg-white border border-pink-200/90 text-pink-950 font-extrabold px-2.5 py-1 rounded-xl text-[11px] shadow-2xs flex items-center gap-1"
                      >
                  ...
                  <p className="text-slate-400 italic text-[11px]">No stay-provided items specified yet.</p>
                )}
              </div>
      ...
      {isModalOpen && (
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Available at this Stay Selection Section */}
            <div>
              <label className="block text-xs font-semibold text-slate-700 tracking-wide uppercase mb-1.5 flex items-center justify-between">
                <span>🏨 Available at this Stay</span>
                <span className="text-[10px] text-slate-500 font-normal">Reflected in Group Checklist</span>
              </label>

              <div className="flex flex-wrap gap-1.5 bg-slate-50 border border-slate-200/80 p-3 rounded-2xl max-h-48 overflow-y-auto">
                {Array.from(new Set([...PREDEFINED_STAY_AMENITIES, ...availableItems])).map((item) => {
                  const isSelected = availableItems.some((a) => a.toLowerCase() === item.toLowerCase());
                  return (
                    <button
                      key={item}
                      type="button"
                      onClick={() => {
                        if (isSelected) {
                          setAvailableItems(availableItems.filter((a) => a.toLowerCase() !== item.toLowerCase()));
                        } else {
              ...
              <div className="flex items-center gap-2 mt-2">
                <input
                  type="text"
                  placeholder="Custom item (e.g. Portable speaker)"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      ...
                      if (customItemInput.trim()) {
                        ...
                        if (!availableItems.some((a) => a.toLowerCase() === newItem.toLowerCase())) {
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 text-slate-900 text-xs rounded-xl p-2.5 focus:outline-none focus:border-pink-500 focus:bg-white font-medium"
                />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="px-3 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs rounded-xl transition-colors shrink-0"
                >
                  + Custom
                </button>
              </div>
