...
export const GroupChatView: React.FC<GroupChatViewProps> = React.memo(({ tripId, tripName, currentUser, isAdmin = false, members = [] }) => {
  ...
  const [expandedSeenMsgId, setExpandedSeenMsgId] = useState<string | null>(null);
      ...
      try {
        ...
        if (data.messages && data.messages.length > 0) {
          setMessages((prev) => {
            ...
            // Avoid unnecessary re-renders if content & read receipts are identical
            const prevKey = prev.map((m) => `${m.id}_${m.content}_${m.isUnsent}_${(m.readByUserIds || []).join(',')}`).join('|');
            const nextKey = merged.map((m) => `${m.id}_${m.content}_${m.isUnsent}_${(m.readByUserIds || []).join(',')}`).join('|');
  ...
  useLayoutEffect(() => {
    ...
    // Never auto-scroll when user has scrolled up to read older messages.
      ...
      {chatTab === 'polls' ? (
        ...
        <>
          ...
          <div
            ...
            className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain p-4 sm:p-5 space-y-4 bg-slate-50/40 relative"
          >
            {isLoading ? (
                ...
                <div key={groupIdx} className="space-y-3">
                  ...
                  {group.msgs.map((msg, msgIdx) => {
                    ...
                    const seenMembers = members
                      ? members
                          .map((m) => m.user)
                          .filter((u) => u.id !== currentUser.id && readList.includes(u.id))
                      : [];
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="max-w-[78%] sm:max-w-[70%] space-y-0.5 relative">
                          ...
                          {/* Instagram-Style Overlapping Avatars for Sender's Messages */}
                          {isMe && !msg.isUnsent && (
                            <div className="flex justify-end mt-0.5 px-0.5">
                              {seenMembers.length > 0 ? (
                                <button
                                  type="button"
                                  onClick={() =>
                                    setExpandedSeenMsgId(expandedSeenMsgId === msg.id ? null : msg.id)
                                  }
                                  className="inline-flex items-center gap-1 cursor-pointer transition-all opacity-90 hover:opacity-100"
                                >
                                  {expandedSeenMsgId === msg.id ? (
                                    <span className="text-[10px] font-extrabold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-full border border-emerald-200 animate-in fade-in duration-100 shadow-2xs">
                                      {seenMembers.map((m) => m.name).join(', ')}
                                    </span>
                                  ) : (
                                    <div className="flex items-center -space-x-1.5 overflow-hidden py-0.5 px-0.5">
                                      {seenMembers.slice(0, 4).map((m) => (
                                        <div
                                          key={m.id}
                                          className="w-4 h-4 rounded-full ring-1 ring-white bg-white flex items-center justify-center overflow-hidden shrink-0 shadow-2xs"
                                          title={m.name}
                                        >
                                          <Avatar name={m.name} size="sm" className="w-4 h-4 text-[8px] font-black border-none shadow-none" />
                                        </div>
                                      ))}
                                      {seenMembers.length > 4 && (
                                        <span className="text-[8px] font-black text-emerald-700 bg-emerald-100 px-1 py-0.2 rounded-full ring-1 ring-white">
                                          +{seenMembers.length - 4}
                                        </span>
                                      )}
                                    </div>
                                ...
                                <div className="flex items-center gap-0.5 text-[9px] text-slate-400 font-semibold opacity-70">
